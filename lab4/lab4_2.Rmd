...
Notice that the `|` operator generates a different result. ### Note: This gives an output almost like either this or this 
```{r}
filter(fish, lakeid == "AL" | length > 350)
```
...
```{r}
new_mammals_2<-filter(mammals, genus %in% c("Lophocebus", "Erythrocebus", "Macaca")
```

```{r}
select(new_mammals_2, "genus", "species", "mass", "gestation", "newborn")
```
