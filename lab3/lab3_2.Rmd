...
## Notes: if you had a spread sheet in excel, you should save it a a .csv file. Easiest to use the data now in R. 
...
```{r}
hot_spring <- readr::read_csv("hsprings_data.csv")
```
...
```
# Notes: 
  The difference between a character and a factor: Factor has specific levels, and a character is like a string of text. 
```{r}
glimpse(hot_springs)
```
